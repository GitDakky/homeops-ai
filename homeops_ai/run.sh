...
if ! command -v hermes >/dev/null 2>&1; then
  ...
  start_homeops_router() {
    ...
    # The Hermes gateway's OpenAI-compatible API server requires bearer
    # auth (api_server.key, generated by Hermes into its config.yaml).
    # Without it every escalated request 401s and voice gets "Invalid
    # API key" instead of an answer. Read the key at start so the router
    # can authenticate its upstream calls.
    GATEWAY_KEY_VALUE=""
    if [ -f /config/.hermes/config.yaml ]; then
      GATEWAY_KEY_VALUE=$(awk '/^[[:space:]]*api_server:/{f=1} f&&/^[[:space:]]*key:/{print $2; exit}' /config/.hermes/config.yaml 2>/dev/null || true)
    fi

...
1. Inspect `/config/.hermes/config.yaml` and confirm the gateway, auth mode, and forced agent defaults.
...
8. Read bundled skills under `/config/.hermes/skills/` before improvising your own workflow.
9. Use `hermes cron list --json` and `hermes system heartbeat last` to understand scheduled behavior before editing automations.
